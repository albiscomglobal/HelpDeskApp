...
<%}} %>
    ...
    user="root" password="password"   />
     ...
     <sql:update dataSource="${webappDataSource}" var="count"  >
           UPDATE customer SET  employeenames=?, employeenumber=?,phonenumber=?, charge_to=?, message=?, priority=? 
            
           WHERE ticket_no= '${param.ticket_no}'
       			 <sql:param value="${param.employeenames}"/>
            <sql:param value="${param.employeenumber}"/>
               <sql:param value="${param.phonenumber}" />
		    <sql:param value="${param.charge_to}"/>
             <sql:param value="${param.message}" />
              <sql:param value="${param.priority}" />
          </sql:update>
       <c:if test="${count>=1}">
       
    <h4 class ="alert alert-success col-md-4 col-md-offset-4" align="center">Record Updated Successfully </h4>
     </c:if>
 ...
 <div class="col-md-6 center-block">
            ...
            <a href="/customermanagement/SuperAdminHomePage.jsp"
						class="btn btn-danger">Back To AdminHomePage</a>
						</div>
           </div>   
